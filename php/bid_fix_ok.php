<?php
	...
	//***************step1******************//
    $sql = "SELECT COUNT(*) AS cnt FROM tbid_ntc_mst WHERE INS_DT LIKE '".date('Y-m')."%'";    
    ...
    if($num < 100)
        $tempBidNo = date('Ymd')."000".$num;
    else if($num < 1000)
        $tempBidNo = date('Ymd')."00".$num;
    else if($num < 10000)
        $tempBidNo = date('Ymd')."0".$num;
    
    $tempBidNo = substr($tempBidNo, 2);
        
    $NTC_NO = "BID".$tempBidNo;                             // 공고번호
    $NTC_CLS_CD = addslashes($_POST['NTC_CLS_CD']);         // 일반
    ...
    $sql1 ="INSERT INTO tbid_ntc_mst SET 
        NTC_NO = '{$NTC_NO}',       
        COMP_CD = '{$COMP_CD}',         
        COMP_NM = '{$COMP_NM}',                                         
        NTC_NM = '{$NTC_NM}',       
        ...
        BID_NTC_YMD = (SELECT CONCAT(DATE_FORMAT(NOW(),'%Y'),DATE_FORMAT(NOW(),'%m'),DATE_FORMAT(NOW(),'%d'))),
        BID_NTC_TM =  (SELECT CONCAT(DATE_FORMAT(NOW(),'%H'),DATE_FORMAT(NOW(),'%i'))),     
    ...
    $sql2 = "INSERT INTO tbid_ntc_crgo SET 
        NTC_NO = '{$NTC_NO}',
        NTC_SEQ = '{$ROUTE_CNT}',
        POL_CD = '{$POL_CD0}',
        POD_CD = '{$POD_CD0}',
    ...
    for($i = 0; $i < $ROUTE_CNT; $i++) 
    {
        ...
        $sql3 = "INSERT INTO tbid_ntc_crgo_route SET 
            NTC_NO = '{$NTC_NO}',            
            POL_CD = '{$POL_CD}',       
            DOOR_NM = '{$DOOR_NM}',
            POD_CD = '{$POD_CD}',
	...
	alertMove('정기입찰 공고가 등록되었습니다.', RT_PATH.'/html/quotation/bid_info.html?NTC_CLS_CD=RGLR');
?>