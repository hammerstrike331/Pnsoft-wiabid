...
{
    // host address
    var $hosts = array("alpha.coolsms.co.kr", "bravo.coolsms.co.kr", "delta.coolsms.co.kr");
    // service port
    var $port = 80;
    ...
    function connect() {
        ...
        foreach ($this->hosts as $host) {
            $connected = $this->__connect__($host, $this->port);
            if ($connected) break;
        }
        return $connected;
    }
    ...
    function send()
    {
        ...
        foreach($this->msgl as $tbsp) {
            ...
            if (array_key_exists("ATTACHMENT", $tbsp)) {
                ...
                $tbsp["ATTACHMENT"] = implode(",", $image_list);
            }
