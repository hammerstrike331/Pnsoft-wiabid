...
$end_date = '2018-03-25 10:00:00';

    if($today_now <= $end_date) {
    //접근을 허용할 ip
        $allow_ips = array(
            "127.0.0.1", "127.0.0.1"
        );
        ...
        if (!in_array($_SERVER['REMOTE_ADDR'], $allow_ips)) {
            echo "
        <script>
        alert('사이트 수정 중입니다. 허용된 IP 만 접근 됩니다.');
...
for ($i=0; $i<$ext_cnt; $i++) {
    ...
    include_once('func.lib.php');    // 공통 함수
...
// ini_set('session.save_path', '/home/financialindex/public_html/session/');
